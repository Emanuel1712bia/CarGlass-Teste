...
public class Program
{
    ...
    public static List<int> EncontrarDivisores(int numero)
    {
        ...
        for (int i = 1; i <= Math.Abs(numero); i++)
        {
            if (numero % i == 0)
            {
                // Se o número for positivo, adiciona só divisores positivos...
                if (numero > 0)
                {
                    divisores.Add(i);
                    divisores.Add(numero / i);
                }
                // Se o número for negativo, adiciona só divisores negativos...
                else
                {
                    divisores.Add(-i);
                    divisores.Add(numero / i); // número/i já será negativo
                }
            }
        }
